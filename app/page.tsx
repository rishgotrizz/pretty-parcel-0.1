...
export default function HomePage() {
  ...
  const featuredProducts = normalProducts.slice(0, 6);
  ...
  return (
    <div className="pb-16">
      ...
      {!loading && specialProducts.length ? (
        <section className="section-shell relative z-0 mt-14 sm:mt-20">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
            {featuredProducts.map((product) => (
              <div key={product._id ?? product.slug} className="flex h-full flex-col gap-4">
                <ProductCard product={product} />
                {product._id ? (
                  <div className="rounded-[1.75rem] border border-pink-100/80 bg-white/80 p-4 shadow-[var(--shadow-card)]">
                    <ProductActions
